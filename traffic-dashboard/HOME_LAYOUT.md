...
The layout is arranged vertically within a centered, responsive container with a consistent gap spacing of `32px`.
...
A high-contrast dark panel serving as the entry hook, positioned at the top of the container.
*   **HTML Element:** `<section className="home-hero">`
*   **Visual Style:** 
    *   Dark background (`#0f172a`) with subtle border (`1px solid #1e293b`).
    *   Centered text alignments with `20px` spacing between title, subtitle, desc, and CTA.
...
*   **Key Components & Interactive Elements:**
    ...
    *   **Tagline (`.hero-tagline`):** High-visibility blue subheading (`#3b82f6`).
    *   **Description (`.hero-desc`):** Mid-tone text (`#94a3b8`) explaining command center features.
    *   **CTA Button (`.btn-predict.btn-hero-cta`):** Styled link to the operations dashboard (`/dashboard`) with an arrow hover animation. Hover style transitions to `#334155` background and lifts up slightly (`transform: translateY(-1px)`).
...
*   **Key Components & Data Binding:**
    ...
    *   **Header (`.stats-card-header`):** Displays icon on the left (e.g. `FaDatabase`, `FaCheckCircle`, `FaFileAlt`, `FaRoad`) followed by the numerical value (`.stats-value`, `28px` font size).
...
*   **Key Components:**
    ...
    *   **Detail Items:** Displays a label on the left (e.g. `Active Incidents`, `Forecast Accuracy`) and highlighted values on the right in bold dark slate.
...
| :--- | :--- | :--- |
| **Tablet Width** (`<= 1024px`) | `@media (max-width: 1024px)` | <ul><li>Dataset statistics strip columns collapse from 4 columns to **2 columns**</li><li>Timeline steps reorganize into a grid of **3 columns**</li></ul> |
| **Mobile Width** (`<= 768px`) | `@media (max-width: 768px)` | <ul><li>Hero title shrinks from `38px` to `30px`</li><li>Hero banner padding decreases to `32px 20px`</li><li>Dataset statistics strip cards stack into a **single column**</li><li>Timeline steps flow vertically (**single column**)</li><li>Dashboard previews collapse from 2 columns to a **single column**</li></ul> |
